...
export default function HealthCentre() {
  ...
  function transferToMyLoft(admission, notes) {
    ...
    const added = birdStore.addBird({
      ...
      originalOwner: stray.ownerDetails || "",
  ...
  return (
    <section className="hospital-centre">
      <header className="hospital-header">
        <div>
          <p className="page-kicker">Health & Biosecurity</p>
          <h2>🏥 Hospital & Quarantine</h2>
          <p>Manage sick birds and incoming strays without mixing their identities.</p>
        </div>
      ...
      <nav className="hospital-navigation" aria-label="Hospital sections">
      ...
      {activeView === "boxes" && <div className="hospital-area-grid">
          ...
          <article className="hospital-area" key={area.id}>
            ...
            <div className="hospital-box-grid">
              {Array.from({ length: area.boxes }, (_, index) => index + 1).map((boxNumber) => {
                ...
                return (
                  <div className={`hospital-box ${admission ? "occupied" : "empty"}`} key={boxNumber}>
                    ...
                    {!admission ? (
                      ...
                      <>
                        ...
                        <div className="hospital-box-actions">
                          <button onClick={() => setOutcomeAdmission(admission)}>Record Outcome</button>
                        </div>
                      </>
                    )}
                  </div>
      ...
      {activeView === "strays" && <section className="hospital-register panel">
        ...
        {hospital.strays.length === 0 ? (
          ...
          <div className="table-wrap">
            <table>
              <thead><tr><th>Stray ID</th><th>Ring Number</th><th>Visits</th><th>Last Arrival</th><th>Last Outcome</th><th>Status</th><th>Loft ID</th></tr></thead>
              <tbody>
                {hospital.strays.map((stray) => (
                  <tr key={stray.strayId}>
                    <td>{stray.strayId}</td><td>{stray.ringNumber}</td><td>{stray.visits.length}</td>
      ...
      {activeView === "owned" && <section className="hospital-register panel">
        ...
        {hospital.admissions.filter((record) => record.birdId).length === 0 ? (
          ...
          <div className="table-wrap"><table>
            ...
            <tbody>{hospital.admissions.filter((record) => record.birdId).map((record) => (
              <tr key={record.id}><td>{record.birdId}</td><td>{record.ringNumber}</td><td>{record.admittedDate}</td><td>{record.reason || "—"}</td><td>{record.status}</td><td>{record.outcome || "—"}</td></tr>
            ))}</tbody>
          </table></div>
        )}
      </section>}
          ...
          <form className="modal" onSubmit={submitIntake}>
            ...
            <div className="form-grid">
              {intakeType === "owned" ? (
                ...
                <><label className="full">Ring Number<input name="ringNumber" value={form.ringNumber} onChange={handleChange} /></label><label className="full">Owner/contact details<input name="ownerDetails" value={form.ownerDetails} onChange={handleChange} /></label></>
              )}
